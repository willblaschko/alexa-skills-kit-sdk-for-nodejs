...
function EmitEvent() {
    ...
    //check to see if we have a contextual (state-based) function for this intent
    if(this.listenerCount(eventString) < 1) { 
        //check if there's a base-state function by this name to fall back to (GLOBAL function)
        if(this.listenerCount(handlerFuncName) > 0){
            eventString = handlerFuncName;            
        }else{
            //else see if Unhandled exists locally or globally
            eventString = 'Unhandled' +
                (this.listenerCount('Unhandled' + this.state) > 0 ? this.state : '');
        }
    }

    if(this.listenerCount(eventString) < 1){
        throw new Error(`In state: ${this.state}. No contextual or global handler function was defined for event ${handlerFuncName} ` +
